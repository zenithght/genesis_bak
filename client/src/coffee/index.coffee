...
$ ->
  ...
  on_open = ->
    ...
    $(document).oneTime '1s', ->
      ...
      resources = [
        { url: '/static/css/heads.png', callback: $.rl.handle_heads }
        { url: '/static/css/poker.png', callback: $.rl.handle_pokers }
        { url: '/static/css/betting.png', callback: $.rl.handle_bets }
      ]

      sounds = ["bet", "raise", "move", "card", "check", "fold", "turn"]
      resources.push {url: '/static/sound/' + s + '.mp3', key: s} for s in sounds
